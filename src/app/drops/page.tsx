...
import { ACTIVE_DROP_QUERY } from '@/lib/sanity/queries'
...
export default async function DropsPage() {
  const drop = await getSanityClient().fetch<SanityDrop>(ACTIVE_DROP_QUERY)
  ...
  return (
    <main style={{ minHeight: '100vh', padding: '60px 24px', maxWidth: '900px', margin: '0 auto' }}>
      ...
      {!drop ? (
        ...
        <Link href={`/drops/${drop.slug.current}`} style={{ textDecoration: 'none', display: 'block' }}>
          <div style={{
